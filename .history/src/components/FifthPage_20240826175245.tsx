import React from 'react'

const FifthPage = () => {
  return (
    <div className='w-full h-[160vh] bg-[#1EC5FC] px-1 pt-10 '>
      {/* this is the first blue area huge text */}
       <div className='text-[4.79rem] px-[3px] tracking-tight leading-[3.6rem] f1 uppercase'>
                              <p className=' overflow-hidden'>We're</p>
                              <p >Providing</p>
                              <p >full- </p>
                              <p className='pl-[7.6rem]'>services </p>
               </div>
      {/* this is the blue area paragraph area  */}
               <div className='w-full gap-10 my-10 f2 text-[23px] tracking-tight leading-[26px] flex flex-col items-center justify-center   '>
                          <div className='text-center opacity-80' >
                                  <p>Positioning</p>
                                  <p>Storytelling</p>
                                  <p>Brand Strategy</p>
                                  <p>Brand identity</p>
                          </div>
                          <div className='text-center opacity-80' >
                                  <p>Art Direction</p>
                                  <p>UX & UI Design</p>
                                  <p>Website & E-Commerce</p>
                                  <p>Mobile App</p>
                          </div>
                          <div className='text-center opacity-80' >
                                  <p>Creative Development</p>
                                  <p>Front-end Development</p>
                                  <p>Back-end Development</p>
                                  <p>Headless CMS</p>
                                  <p>Wordpress</p>
                          </div>
               </div>

               <div className='text-[4.79rem] px-[3px] tracking-tight leading-[3.6rem] f1 uppercase'>
                              <p className=' overflow-hidden'>from</p>
                              <p >strategy </p>
                              <p className='pl-10'>to Design</p>
                              <p className='pl-[7.6rem]'>and</p>
               </div>
    </div>
  )
}

export default FifthPage